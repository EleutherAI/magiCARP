...
from vis_util import scatter_with_names, spherical_coord
...
def make_batch_elem(tok_out):
    return BatchElement(tok_out["input_ids"], tok_out["attention_mask"])
...
def encode_rand_batch(size):
    ...
    with torch.no_grad():
        pass_encs, rev_encs = cloob_model.calculate_embeddings([p_batch], [r_batch])
    # -> [[pass_encs], [rev_encs]]
    ...
    return (pass_encs, p_batch_s), (rev_encs, r_batch_s)
