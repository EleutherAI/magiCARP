...
reviews = carp_model.review_encoder.call_tokenizer(\
    ...
    list(map(lambda x: '[quote] '+x,\
        get_review_ensemble(reviews))), [])).to("cuda")
...
with torch.no_grad():
    pass_encs, rev_encs =\
        carp_model.calculate_embeddings([passage_batch], [review_batch])
    # compute cosine similarity and average over the last dimension
    confusion_matrix = torch.mean(carp_model.cosine_sim(pass_encs[0], rev_encs[0]).reshape(
        passages.input_ids.shape[0], review_count, num_beams), dim=-1)
