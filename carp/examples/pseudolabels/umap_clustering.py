...
if __name__ == "__main__":
    ...
    try:
        ...
        hdbscan_clusterer = joblib.load(
            "carp/examples/pseudolabels/hdbscan_clusterer.joblib"
        )
        ...
        hdbscan_clusterer = HDBSCAN(
            min_samples=5, min_cluster_size=30, prediction_data=True
        ).fit(review_encs)

        joblib.dump(
            hdbscan_clusterer, "carp/examples/pseudolabels/hdbscan_clusterer.joblib"
        )
    ...
    y = review_encs[:, 1]
    c = np.where(
        labels != -1, labels / labels.max(), labels
    )  # normalize non error labels to [0, 1] for color
