...
class DistillCARP(BaseModel):
	...
	def train_step(
		...
		rev_mbs: List[DistillBatchElement] = [
			DistillBatchElement(reviews.input_ids[i], reviews.mask[i], reviews.reviews_per_passage) for i in microbatch_inds
		]
		...
		for index, passage in enumerate(pass_mbs):
			...
			with torch.cuda.amp.autocast():
				...
				loss  = self.reduced_contrastive_loss(
					torch.cat(pass_tmp), torch.cat(rev_encs), reviews_per_passage
				)
		...
		for index, review in enumerate(rev_mbs):
			...
			with torch.cuda.amp.autocast():
				...
				loss = self.reduced_contrastive_loss(
					torch.cat(pass_encs), torch.cat(rev_tmp), reviews_per_passage
				)
			scaler.scale(loss).backward()
		# Clipping
